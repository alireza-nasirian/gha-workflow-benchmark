#!/usr/bin/env python3
"""
Batch Poutine Analysis with Docker - STABLE VERSION
Uses one container per analysis (more stable) + progress tracking.
Slightly slower but much more reliable when Docker has issues.
"""
...
# Import from original Docker version
from poutine_docker import (
    CommitInfo, PoutineResult, ComparisonResult,
    PoutineDockerAnalyzer, load_commits_from_jsonl, load_commits_from_csv
)
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch Poutine analysis with Docker - stable version (one container per analysis)'
    )
    ...
    print("="*70)
    print("BATCH POUTINE ANALYSIS (DOCKER - STABLE)")
    print("="*70)
    print(f"Data root: {args.data_root}")
    print(f"Input file: {args.input}")
    print(f"Output directory: {args.output}")
    print(f"Batch size: {args.batch_size}")
    print("\n✓ Stable version - new container per analysis (more reliable!)\n")
    ...
    # Initialize Docker analyzer (uses one container per analysis - stable!)
    print("\nInitializing Docker analyzer...")
